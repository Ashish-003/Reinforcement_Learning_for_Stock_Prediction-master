...
class YahooFinance:
    ...
    def _parsing_json(self, data):
        ...
        timestamps = [_time.strftime(
            '%a, %d %b %Y %H:%M:%S', _time.localtime(x)) for x in timestamps]
    ...
    def to_csv(self, file_name,modeflag,headerflag):
        self.result.to_csv(file_name, mode=modeflag, header=headerflag)
        return self._result
    def check(self):
        return self._result
